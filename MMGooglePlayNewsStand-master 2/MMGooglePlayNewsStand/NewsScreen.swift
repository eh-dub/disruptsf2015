...
class MMSampleTableViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,MMPlayPageScroll ,UIScrollViewDelegate{
    ...
    */
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if(displayEntities == false){
            //over here, i should restart the view controller to try to load new shit
            key = titleToPass[indexPath.row]
            dataHelper.initializeView(key, vc: self)
            println(titleToPass[0])
            displayEntities = true
        }
        else {
            currentEntite = entities[indexPath.row]
